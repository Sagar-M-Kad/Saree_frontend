...
import { sarees, categories, formatPrice } from "@/data/sarees";
    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto flex max-w-7xl flex-col gap-8 px-5 py-16 sm:px-8 sm:py-24 lg:py-32">
          <div className="reveal-up flex items-center gap-2 text-xs uppercase tracking-[0.32em] text-primary" style={{ animationDelay: '100ms' }}>
            <Sparkles className="h-3.5 w-3.5" />
            The AI Saree Atelier
          </div>
      ...
      >
        <div className="flex items-end justify-between gap-6 pb-6">
          <div>
            ...
            <h2 className="mt-2 font-serif text-3xl sm:text-4xl">Saree Atelier</h2>
          </div>
          ...
          {filtered.map((s, i) => (
            ...
            >
              <div className="relative aspect-[3/4] overflow-hidden">
                ...
                <span className="absolute left-4 top-4 glass px-4 py-1.5 text-[10px] uppercase tracking-[0.2em] font-semibold text-primary rounded-full">
                  {s.category}
                </span>
              </div>
              <div className="absolute inset-x-0 bottom-0 p-6">
                <div className="flex items-end justify-between gap-3">
                  <div className="min-w-0">
                    <h3 className="truncate font-serif text-3xl text-foreground">{s.name}</h3>
                    <p className="truncate text-sm text-muted-foreground mt-1">{s.subtitle}</p>
                  </div>
                  <div className="shrink-0 text-right">
                    <div className="font-serif text-2xl text-primary">
                      {formatPrice(s.price)}
                    </div>
                    <div className="text-[10px] uppercase tracking-[0.2em] text-primary/80 mt-1">
                      Try-on ready
                    </div>
                  </div>
                </div>
                <div className="mt-5 inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-primary opacity-0 transition-all duration-300 translate-y-2 group-hover:opacity-100 group-hover:translate-y-0">
